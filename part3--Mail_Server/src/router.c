...
static int jw_reserve(json_writer_t *jw, size_t extra) {
    ...
    char *tmp = realloc(jw->buf, new_cap);
    if (!tmp) return -1;
    jw->buf = tmp;
    jw->cap = new_cap;
    return 0;
...
static int json_parse(const char *json, jsmntok_t **tokens_out, int *count_out) {
    ...
    jsmn_parser parser;
    jsmn_init(&parser);
    jsmntok_t *tokens = calloc(JSON_TOKEN_COUNT, sizeof(jsmntok_t));
    if (!tokens) return -1;
...
static int query_get_param(const char *query, const char *key, char *out, size_t out_len) {
    ...
    while (*ptr) {
        ...
        const char *eq = memchr(ptr, '=', chunk_len);
        if (eq) {
...
static void handle_message_compose(server_runtime_t *rt, http_request_t *req, http_response_t *res) {
    ...
    compose_request_t compose = {
        ...
        .custom_folder = custom[0] ? custom : NULL,
        .archive_group = archive_group[0] ? archive_group : NULL,
